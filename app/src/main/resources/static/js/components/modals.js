...
export function openModal(type) {
  const { modal, modalBody, closeModal } = getModalElements();
  ...
  if (type === "patientSignup") {
    document.getElementById("signupBtn")?.addEventListener("click", () => {
      if (window.signupPatient) window.signupPatient();
    });
  }

  if (type === "patientLogin") {
    document.getElementById("loginBtn")?.addEventListener("click", () => {
      if (window.loginPatient) window.loginPatient();
    });
  }

  if (type === "addDoctor") {
    document.getElementById("saveDoctorBtn")?.addEventListener("click", () => {
      if (window.adminAddDoctor) window.adminAddDoctor();
    });
  }

  if (type === "adminLogin") {
    document.getElementById("adminLoginBtn")?.addEventListener("click", () => {
      if (window.adminLoginHandler) window.adminLoginHandler();
    });
  }

  if (type === "doctorLogin") {
    document.getElementById("doctorLoginBtn")?.addEventListener("click", () => {
      if (window.doctorLoginHandler) window.doctorLoginHandler();
    });
  }
}
